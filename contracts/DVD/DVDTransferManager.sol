...
contract DVDTransferManager is Ownable {
    ...
    /**
     * @dev Emitted when a DVD transfer is initiated by `maker` to swap `token1Amount` tokens `token1` 
     ...
     * executed either by `taker` either by the agent of the ERC3643 token
     * if the ERC3643 token is subject to conditional transfers
    ...
    /**
     *  @dev modify the fees applied to a parity of tokens (tokens can be ERC3643 or ERC20)
     ...
     *  `_token1` and `_token2` need to be ERC20 or ERC3643 tokens addresses, otherwise the transaction will fail
     *  `msg.sender` has to be owner of the DVD contract or the owner of the ERC3643 token involved in the parity (if any)
    ...
    function modifyFee(
        ...
        require(msg.sender == owner() || isERC3643Owner(_token1, msg.sender) || isERC3643Owner(_token2, msg.sender), "Ownable: only owner can call");
     ...
     *  @param _token1 the address of the token (ERC20 or ERC3643) provided by `msg.sender`
     ...
     *  @param _token2 the address of the token (ERC20 or ERC3643) provided by `_counterpart`
     ...
     *  requires that `msg.sender` is the taker OR the ERC3643 agent in case a
    ...
    function takeDVDTransfer(bytes32 _transferID) external {
        ...
        require(
            msg.sender == token2.counterpart || isERC3643Agent(token1.token, msg.sender) || isERC3643Agent(token2.token, msg.sender),
     ...
     *  owner or the ERC3643 agent in case a ERC3643 token is involved in the transfer
    ...
    function cancelDVDTransfer(bytes32 _transferID) external {
        ...
        require(
            msg.sender == token2.counterpart ||
                ...
                msg.sender == owner() ||
                isERC3643Agent(token1.token, msg.sender) ||
                isERC3643Agent(token2.token, msg.sender),
     ...
     *  the token is a ERC3643, otherwise it's not a ERC3643
     *  return `true` if the token is a ERC3643, `false` otherwise
     */
    ...
    /**
     *  @dev check if `_user` is a ERC3643 agent of `_token`
     *  @param _token the address token to check
     *  @param _user the wallet address
     *  if `_token` is a ERC3643 token this function will check if `_user` is registered as an agent on it
     *  return `true` if `_user` is agent of `_token`, return `false` otherwise
     */
    function isERC3643Agent(address _token, address _user) public view returns (bool) {
        if (isERC3643(_token)) {
    ...
    /**
     *  @dev check if `_user` is a ERC3643 owner of `_token`
     *  @param _token the address token to check
     *  @param _user the wallet address
     *  if `_token` is a ERC3643 token this function will check if `_user` is registered as an owner on it
     *  return `true` if `_user` is owner of `_token`, return `false` otherwise
     */
    function isERC3643Owner(address _token, address _user) public view returns (bool) {
        if (isERC3643(_token)) {
    ...
    function calculateFee(bytes32 _transferID) public view returns (TxFees memory) {
        ...
        if (feeDetails.token1Fee != 0 || feeDetails.token2Fee != 0) {
            uint256 _txFee1 = (token1.amount * feeDetails.token1Fee * 10**(feeDetails.feeBase - 2)) / (10**feeDetails.feeBase);
            uint256 _txFee2 = (token2.amount * feeDetails.token2Fee * 10**(feeDetails.feeBase - 2)) / (10**feeDetails.feeBase);
            fees.txFee1 = _txFee1;
            fees.txFee2 = _txFee2;
