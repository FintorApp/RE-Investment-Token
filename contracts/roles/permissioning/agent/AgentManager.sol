...
contract AgentManager is AgentRoles {
     ...
     *  Requires that `_identityID` is set as TransferManager on the AgentManager contract
    ...
    function callForcedTransfer(
        ...
        require(
            isTransferManager(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
     ...
     *  AgentManager has to be set as agent on the token smart contract to process this function
     *  See {IToken-pause}.
     *  Requires that `_identityID` is set as Freezer on the AgentManager contract
     *  Requires that msg.sender is a MANAGEMENT KEY on `_identityID`
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callPause(IIdentity _identityID) external {
        require(isFreezer(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2), "Role: Sender is NOT Freezer");
        token.pause();
    }
     ...
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callUnpause(IIdentity _identityID) external {
        require(isFreezer(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2), "Role: Sender is NOT Freezer");
        token.unpause();
    }
     ...
     *  AgentManager has to be set as agent on the token smart contract to process this function
     *  See {IToken-mint}.
     *  Requires that `_identityID` is set as SupplyModifier on the AgentManager contract
     *  Requires that msg.sender is a MANAGEMENT KEY on `_identityID`
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callMint(
        ...
        require(
            isSupplyModifier(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
     ...
     *  Requires that `_managerIdentityID` is set as RecoveryAgent on the AgentManager contract
     *  Requires that msg.sender is a MANAGEMENT KEY on `_managerIdentityID`
     *  @param _managerIdentityID the identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callRecoveryAddress(
        address _lostWallet,
        address _newWallet,
        address _identityID,
        IIdentity _managerIdentityID
    ) external {
        require(
            isRecoveryAgent(address(_managerIdentityID)) && _managerIdentityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
            "Role: Sender is NOT Recovery Agent"
        );
        token.recoveryAddress(_lostWallet, _newWallet, _identityID);
    }
     ...
     *  Requires that `ManagerIdentityID` is set as WhiteListManager on the AgentManager contract
     *  Requires that msg.sender is a MANAGEMENT KEY on `_managerIdentityID`
     *  @param _managerIdentityID the identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callRegisterIdentity(
        ...
        require(
            isWhiteListManager(address(_managerIdentityID)) && _managerIdentityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
            "Role: Sender is NOT WhiteList Manager"
        );
        token.identityRegistry().registerIdentity(_userAddress, _identityID, _country);
    }
     ...
     *  Requires that `_identityID` is set as WhiteListManager on the AgentManager contract
     *  Requires that msg.sender is a MANAGEMENT KEY on `_identityID`
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callUpdateIdentity(
        ...
        require(
            isWhiteListManager(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
            "Role: Sender is NOT WhiteList Manager"
        );
     ...
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callDeleteIdentity(address _userAddress, IIdentity _identityID) external {
        require(
            isWhiteListManager(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
            "Role: Sender is NOT WhiteList Manager"
        );
