...
contract OwnerManager is OwnerRoles {
     ...
     *  Requires that `_identityID` is set as RegistryAddressSetter on the OwnerManager contract
     *  Requires that msg.sender is an ACTION KEY on `_identityID`
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callSetIdentityRegistry(address _identityRegistry, IIdentity _identityID) external {
        require(
            isRegistryAddressSetter(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
     ...
     *  Requires that `_identityID` is set as ComplianceSetter on the OwnerManager contract
     *  Requires that msg.sender is a MANAGEMENT KEY on `_identityID`
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callSetCompliance(address _compliance, IIdentity _identityID) external {
        require(
            isComplianceSetter(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
     ...
     *  Requires that `_identityID` is set as ComplianceManager on the OwnerManager contract
     *  Requires that msg.sender is an ACTION KEY on `_identityID`
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callComplianceFunction(bytes calldata callData, IIdentity _identityID) external {
        require(
            isComplianceManager(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
     ...
     *  OwnerManager has to be set as owner on the token smart contract to process this function
     *  See {IToken-setName}.
     *  Requires that `_identityID` is set as TokenInfoManager on the OwnerManager contract
     *  Requires that msg.sender is an ACTION KEY on `_identityID`
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callSetTokenName(string calldata _name, IIdentity _identityID) external {
        require(
            isTokenInfoManager(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
     ...
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callSetTokenSymbol(string calldata _symbol, IIdentity _identityID) external {
        require(
            isTokenInfoManager(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
            "Role: Sender is NOT Token Information Manager"
        );
    ...
    /**
     *  @dev calls the `setIdentityID` function on the token contract
     *  OwnerManager has to be set as owner on the token smart contract to process this function
     *  See {IToken-setIdentityID}.
     *  Requires that `_tokenIdentityID` is set as TokenInfoManager on the OwnerManager contract
     *  Requires that msg.sender is an ACTION KEY on `_identityID`
     *  @param _identityID the identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callSetTokenIdentityID(address _tokenIdentityID, IIdentity _identityID) external {
        require(
            isTokenInfoManager(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
            "Role: Sender is NOT Token Information Manager"
        );
        token.setIdentityID(_tokenIdentityID);
    }
     ...
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callSetClaimTopicsRegistry(address _claimTopicsRegistry, IIdentity _identityID) external {
        require(
            isRegistryAddressSetter(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
            "Role: Sender is NOT Registry Address Setter"
        );
     ...
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callSetTrustedIssuersRegistry(address _trustedIssuersRegistry, IIdentity _identityID) external {
        require(
            isRegistryAddressSetter(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
            "Role: Sender is NOT Registry Address Setter"
        );
     ...
     *  Requires that `_identityID` is set as IssuersRegistryManager on the OwnerManager contract
     *  Requires that msg.sender is an ACTION KEY on `_identityID`
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callAddTrustedIssuer(
        ...
        IIdentity _identityID
    ) external {
        require(
            isIssuersRegistryManager(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
     ...
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callRemoveTrustedIssuer(IClaimIssuer _trustedIssuer, IIdentity _identityID) external {
        require(
            isIssuersRegistryManager(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
            "Role: Sender is NOT IssuersRegistryManager"
        );
     ...
     *  Requires that `_identityID` is set as ClaimRegistryManager on the OwnerManager contract
     *  Requires that msg.sender is an ACTION KEY on `_identityID`
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callAddClaimTopic(uint256 _claimTopic, IIdentity _identityID) external {
        require(
            isClaimRegistryManager(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
     ...
     *  @param _identityID the _identityID contract of the caller, e.g. "i call this function and i am Bob"
     */
    function callRemoveClaimTopic(uint256 _claimTopic, IIdentity _identityID) external {
        require(
            isClaimRegistryManager(address(_identityID)) && _identityID.keyHasPurpose(keccak256(abi.encode(msg.sender)), 2),
            "Role: Sender is NOT ClaimRegistryManager"
        );
